function sys = heuristic_system_setup(sys)
...
%% find Q_tilde
QQ = [... % this is the Q_tilde we found after many trials in the large-scale model
    18;
    25;
    36;
    42;
...
if p < 30
    ...
    % use [Q_tilde,avg_rank] = get_Q_tilde(sys,p_tilde,QQ);
    % instead of [Q_tilde,avg_rank] = get_Q_tilde(sys,p_tilde);
    % and then search manually
    Q_tilde = QQ;
    p_tilde = length(QQ);
end
Q_tilde
...
sing_Phi = svd(Phi);
smin = sing_Phi(min(Phi_rows,Nsamples));
...
function [Q_tilde,avg_rank] = get_Q_tilde_(sys,p_tilde,QQ)
...
% Maximum number of random configurations
maxTrials = min(1,100*p);

parfor k = 1:maxTrials

    % Random subset
    %Q = randperm(p,p_tilde);
    %Q = QQ(randperm(numel(QQ), p_tilde));
    Q = QQ;
    ...
    if observable
        ...
        fprintf('FOUND\n')
        Q_tilde
        %avg_rank = round(avg_rank/k);
        %return
    end

end

%avg_rank = round(avg_rank/maxTrials);

% No observable subset found
%Q_tilde = [];
